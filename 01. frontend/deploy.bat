...
rem  deploy.bat: construye la imagen Docker (build de Vite + nginx) y levanta
rem  el contenedor, conectado a la misma red que el backend (ver
rem  02. backend\deploy.bat), del cual hace de reverse proxy para /api.
rem
...
echo [1/4] Verificando red Docker "%NETWORK_NAME%" ...
docker network inspect %NETWORK_NAME% >nul 2>&1
if errorlevel 1 (
  docker network create %NETWORK_NAME%
)

echo [2/4] Construyendo imagen %IMAGE_NAME% ...
docker build -t %IMAGE_NAME% .
if errorlevel 1 (
  echo [ERROR] Fallo la construccion de la imagen del frontend.
  exit /b 1
)
